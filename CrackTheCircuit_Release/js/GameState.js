...
// Define minimum and maximum levels (adjust as needed)
var MIN_LEVEL = 14;
var MAX_LEVEL = 18; // Allowed level range 1 to 10
...
CrackTheCircuit.GameState.prototype = {
  ...
  loadSavedData: function () {
    if (this.privateMode) {
      this.numLevelsBeaten = MIN_LEVEL - 1;
      this.numTutsComplete = 0;
      return;
    }

    // Use MIN_LEVEL - 1 as the starting point, so the first level is MIN_LEVEL
    if (localStorage.getItem("numLevelsBeaten") == null) {
      this.numLevelsBeaten = MIN_LEVEL - 1;
      localStorage.setItem("numLevelsBeaten", MIN_LEVEL - 1);
    } else {
      let storedLevel = parseInt(localStorage.getItem("numLevelsBeaten"));
      // Clamp the stored level between MIN_LEVEL - 1 and MAX_LEVEL
      if (storedLevel < MIN_LEVEL - 1) {
        storedLevel = MIN_LEVEL - 1;
      } else if (storedLevel > MAX_LEVEL) {
        storedLevel = MAX_LEVEL;
      }
      this.numLevelsBeaten = storedLevel;
    }
  ...
  loadLevelData: function () {
    // Calculate the next level to load based on beaten levels.
    this.curLevelNum = this.numLevelsBeaten + 1;

    // Enforce allowed range: if current level is outside the allowed levels, reset to MIN_LEVEL.
    if (this.curLevelNum < MIN_LEVEL || this.curLevelNum > MAX_LEVEL) {
      this.curLevelNum = MIN_LEVEL;
    }

    this.gameData = JSON.parse(this.game.cache.getText("gameData"));
    this.totalLevels = this.gameData.levels.length;

    // If the stored levels exceed the total levels in your game data, reset.
    if (this.numLevelsBeaten >= this.totalLevels) {
      this.curLevelNum = MIN_LEVEL;
    }
  },
  ...
  saveGameData: function () {
    ...
    // Ensure numLevelsBeaten is within allowed bounds before saving.
    if (this.numLevelsBeaten < MIN_LEVEL - 1) {
      this.numLevelsBeaten = MIN_LEVEL - 1;
    } else if (this.numLevelsBeaten > MAX_LEVEL) {
      this.numLevelsBeaten = MAX_LEVEL;
    }
  ...
  positionTableItemsForMainMenu: function () {
    ...
    this.pencil.position = {
      x: 0.77 * this.game.world.width,
      y: 0.58 * this.game.world.height,
    };
    this.pencil.angle = -64;
    this.protractor.position = {
      x: 0.21 * this.game.world.width,
      y: 0.66 * this.game.world.height,
    };
  ...
  moveTableItemsToStateFromStateAfterDelay: function (gameStateTo, gameStateFrom, delay) {
    if (gameStateTo == CrackTheCircuit.STATE_MAIN_MENU) {
      if (gameStateFrom == CrackTheCircuit.STATE_FREE_DRAW) {
        this.game.add.tween(this.protractor.scale).to({ x: 0.65, y: 0.65 }, 1000, "Sine.easeInOut", true, delay);
        this.game.add.tween(this.protractor).to(
          {
            x: 0.21 * this.game.world.width,
            y: 0.66 * this.game.world.height,
          },
        ...
        this.game.add.tween(this.pencil).to(
          {
            x: 0.77 * this.game.world.width,
            y: 0.58 * this.game.world.height,
          },
        ...
        //if(gameStateFrom == CrackTheCircuit.STATE_GAME)
        this.game.add.tween(this.protractor).to(
          {
            x: 0.21 * this.game.world.width,
            y: 0.66 * this.game.world.height,
      ...
      this.game.add.tween(this.protractor.scale).to({ x: 0.77844, y: 0.77844 }, 1000, "Sine.easeInOut", true, delay);
      this.game.add.tween(this.protractor).to({ x: 0.16 * this.game.world.width, y: 0.28 * this.game.world.height }, 1000, "Sine.easeInOut", true, delay);
  ...
  playBtnPressed: function () {
    ...
    if (this.levelToLoad > this.totalLevels) {
      this.levelToLoad = this.curLevelNum;
    }
    if (this.levelToLoad > this.totalLevels) {
      this.levelToLoad = 1;
    }
  ...
  raiseTitleBoxAgainAfterDelay: function (delay) {
    //when back to menu, after initial loading
    this.fadeShade.destroy(); //called after shade has finished fading out
  ...
  lowerTitleBox: function () {
    ...
    if (this.hasLoadedGameAssets) {
      ...
      //if game assets have not been loaded yet
  ...
  createBlueprint: function () {
    ...
    this.blueprint.position = {
      x: this.game.world.centerX,
      y: 1.4 * this.game.world.height,
    };
  ...
  showBlueprint: function () {
    ...
    this.componentCards.position = {
      x: 0.81 * this.game.world.width,
      y: 0.73 * this.game.world.height,
    }; //in case it tweened away earlier
  ...
  addCardOfType: function (type) {
    if (this.gameState == CrackTheCircuit.STATE_GAME || type == CrackTheCircuit.COMPONENT_BATTERY) {
      //not free draw
  ...
  hideCardsAfterDelay: function (delay) {
    ...
    var cardTween = this.game.add.tween(this.componentCards).to(
      {
        x: this.componentCards.position.x + 100,
        y: 1.5 * this.game.world.height,
      },
      500,
      "Sine.easeIn",
      true,
  ...
  circuitSolvedInBlueprint: function () {
    if (this.gameState == CrackTheCircuit.STATE_GAME && !this.tutorialShowing && this.componentCards.allCardsUsedUp() && !this.blueprint.isDraggingComponent) {
      //second arg to prevent circuit from checking when switch is unsnapped and dragging (crash)
      var corct1 = 0,
        corct2 = 0;
      if (this.blueprint.allSwitchesClosed()) {
        ...
        //console.log(corct1);
        if (corct1) {
          this.menu.showPasswordPopup((isCorrect) => {
            if (isCorrect) {
              //console.log("WIN");
              this.initLevelWin();
            }
  ...
  initLevelWin: function () {
    ...
    if (this.numLevelsBeaten < this.curLevelNum) {
      //level not yet beaten
      ...
      this.menu.fadeInCompletionText(true, false, 1400);
    }
    ...
    if (this.winKey) {
      this.game.world.bringToTop(this.winKey);
    }
  ...
  fadeInWinGlowAfterDelay: function (delay) {
    this.winGlow.position = {
      x: this.blueprint.position.x,
      y: this.blueprint.position.y,
    };
  ...
  fadeInWinRaysAfterDelay: function (delay) {
    this.winRaysCW.position = {
      x: this.blueprint.position.x,
      y: this.blueprint.position.y,
    };
    this.winRaysCCW.position = {
      x: this.blueprint.position.x,
      y: this.blueprint.position.y,
    };
  ...
  updateWinRayRotationScale: function () {
    this.winRaysCW.forEach(function (ray) {
      ...
      ray.scale.setTo(
        -(CrackTheCircuit.WIN_RAY_MAX_SCALE - CrackTheCircuit.WIN_RAY_MIN_SCALE) * Math.cos(((2 * Math.PI) / 180) * ray.angle) +
          (CrackTheCircuit.WIN_RAY_MIN_SCALE + CrackTheCircuit.WIN_RAY_MAX_SCALE) / 2
      );
    }, this);
  ...
  showBlueprintForFreeDraw: function () {
    ...
    this.blueprint.visible = true;
    this.blueprint.position = {
      x: 0.46 * this.game.width,
      y: 1.6 * this.game.height,
    };
    ...
    this.componentCards.position = {
      x: 0.89 * this.game.world.width,
      y: 0.5 * this.game.world.height,
    }; //in case it tweened away earlier
