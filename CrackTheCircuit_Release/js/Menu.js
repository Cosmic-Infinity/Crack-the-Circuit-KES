...
Menu.prototype.createSoundMusicBtns = function () {
  var soundBtnString;
  if (this.soundOn) {
    soundBtnString = "sound_btn";
  } else {
    soundBtnString = "sound_btn_x";
  }
  ...
  this.soundBtn.customParams = {
    btnIndex: CrackTheCircuit.BTN_INDEX_SOUND,
    spriteString: soundBtnString,
  };
...
Menu.prototype.createBackBtn = function () {
  ...
  this.backToMenuBtn.customParams = {
    btnIndex: CrackTheCircuit.BTN_INDEX_BACK,
    spriteString: "menu_btn",
    isText: false,
  };
...
Menu.prototype.fadeOutBackButton = function () {
  ...
  if (this.gameState.gameState == CrackTheCircuit.STATE_MAIN_MENU) {
    backBtnFadeOut.onComplete.add(this.fadeInMenuButtons, this);
  }
};
...
Menu.prototype.createTrashBtn = function () {
  ...
  this.trashBtn.customParams = {
    btnIndex: CrackTheCircuit.BTN_INDEX_TRASH,
    spriteString: "button_trash",
    isText: false,
  };
...
Menu.prototype.fadeInTrashButton = function () {
  if (this.gameState.gameState == CrackTheCircuit.STATE_FREE_DRAW) {
    this.trashBtn.position = {
      x: 0.0012 * this.game.world.width,
      y: 0.11 * this.game.world.height,
    };
    this.trashBtn.scale.setTo(0.55);
  } else {
    this.trashBtn.position = {
      x: 0.138 * this.game.world.width,
      y: 0.505 * this.game.world.height,
    };
...
Menu.prototype.createSizeBtn = function () {
  ...
  this.sizeBtn.customParams = {
    btnIndex: CrackTheCircuit.BTN_INDEX_SIZE,
    spriteString: "button_size",
    isText: false,
  };
...
Menu.prototype.createFullScreenXBtn = function () {
  ...
  this.fullScreenXBtn.customParams = {
    btnIndex: CrackTheCircuit.BTN_INDEX_FULL_SCREEN_X,
    spriteString: "button_x",
    isText: false,
  };
...
Menu.prototype.createHintBtn = function () {
  ...
  this.hintBtnGroup.position = {
    x: 0.045 * this.game.width,
    y: 0.187 * this.game.height,
  };
  ...
  this.hintBtn.customParams = {
    btnIndex: CrackTheCircuit.BTN_INDEX_HINT,
    spriteString: "hint_btn",
    isText: false,
  };
...
Menu.prototype.enableHintButton = function () {
  ...
  this.hintBtnGlow.visible = true;
  this.game.add.tween(this.hintBtnGroup.scale).to(
    {
      x: this.hintBtnGroup.scale.x * CrackTheCircuit.HINT_BTN_SCALE_FACTOR_ACTIVE,
      y: this.hintBtnGroup.scale.y * CrackTheCircuit.HINT_BTN_SCALE_FACTOR_ACTIVE,
    },
    150,
...
Menu.prototype.createMenuButtons = function () {
  ...
  this.playBtn = this.game.add.bitmapText(this.game.world.centerX, 0.7 * this.game.world.height, "menuFont", "New Game", 86);
  this.playBtn.anchor.setTo(0.5);
  this.playBtn.customParams = {
    btnIndex: CrackTheCircuit.BTN_INDEX_PLAY,
    isText: true,
  };
  ...
  //   this.selectLevelBtn.customParams = {
  //     btnIndex: CrackTheCircuit.BTN_INDEX_SELECT_LEVEL,
  //     isText: true,
  //   };
...
Menu.prototype.enableMenuButtons = function () {
  ...
  //if((this.gameState.numLevelsBeaten-13) == 0) { this.selectLevelBtn.inputEnabled = false; }
};
...
Menu.prototype.btnOnOver = function (sprite, pointer) {
  if (this.mouseDownOnButtonIndex == sprite.customParams.btnIndex) {
    //the mouse was down on btn and moved away and back
  ...
  } else if (this.isTouchScreen || this.mouseDownOnButtonIndex != -1) {
    //the mouse was down on another button, then moved
    //do nothing
  } else {
    if (sprite.customParams.isText) {
      sprite.font = "menuFontHighlighted";
...
Menu.prototype.btnOnUp = function (sprite, pointer, isOver) {
  ...
  if (isOver && sprite.visible) {
    ...
    if (sprite.customParams.btnIndex == CrackTheCircuit.BTN_INDEX_MUSIC) {
      if (this.musicOn) {
        sprite.customParams.spriteString = "music_btn_x";
      } else {
        sprite.customParams.spriteString = "music_btn";
      }
    } else if (sprite.customParams.btnIndex == CrackTheCircuit.BTN_INDEX_SOUND) {
      if (this.soundOn) {
        sprite.customParams.spriteString = "sound_btn";
      } else {
        sprite.customParams.spriteString = "sound_btn_x";
      }
    }
  }
};
...
Menu.prototype.nextLevelPressed = function () {
  if (this.gameState.curLevelNum == this.gameState.totalLevels) {
    //if last level, back to menu
    this.backBtnPressed();
  } else {
    this.fadeOutBackButton();
    this.hideWinButtons();
...
Menu.prototype.fadeInMenuButtons = function () {
  ...
  if (this.gameState.numLevelsBeaten - 13 == 0) {
    playString = "New Game";
...
Menu.prototype.fadeInCompletionText = function (showKey, inMenu, delay) {
  this.completionText.text = this.gameState.numLevelsBeaten - 13 + "/" + (this.gameState.totalLevels - 13);
  var posX, posY;
  if (inMenu) {
  ...
  if (!showKey) {
    //if it is being shown from the win screen and the score is being incremented
    //increment when key hits score
    this.completionText.text = this.gameState.numLevelsBeaten - 13 + "/" + (this.gameState.totalLevels - 13); //do it twice so when it changes, it is in the correct position
  }
  ...
  if (showKey) {
    this.keySymbol.visible = true;
    if (inMenu) {
      this.keySymbol.position = {
        x: this.completionText.position.x,
        y: this.completionText.position.y - 0.5 * this.completionText.height - 3,
      };
    } else {
      this.keySymbol.position = {
        x: this.completionText.position.x,
...
Menu.prototype.createShortHelpBtn = function () {
  ...
  this.shortHelpBtn.customParams = {
    btnIndex: CrackTheCircuit.BTN_INDEX_SHORT_HELP,
    spriteString: "shortHelp_btn",
    isText: false,
  };
  ...
  this.shortHelpBtn.position = {
    x: this.game.world.width - 0.5 * this.shortHelpBtn.width - 10,
    y: this.game.world.height - 0.5 * this.shortHelpBtn.height - 8,
  };
...
Menu.prototype.showShortHelpBtn = function () {
  if (!this.shortHelpBtn) {
    this.createShortHelpBtn();
  }
...
Menu.prototype.createLevelSelectPage = function () {
  ...
  this.levelSelectPageGroup.position = {
    x: this.game.world.centerX,
    y: 1.3 * this.game.world.height,
  };
  ...
  for (var i = 1; i <= this.totalLevels; i++) {
    ...
    if (i <= this.gameState.numLevelsBeaten) {
      //the level has already been beaten
    ...
    if (i > this.gameState.numLevelsBeaten + 1) {
      //the level is locked
  ...
  this.levelSelectArrowRight.customParams = {
    btnIndex: CrackTheCircuit.BTN_INDEX_LEVEL_SELECT_NEXT,
    spriteString: "levelSelectArrow",
  };
  this.game.add
    .tween(this.levelSelectArrowRight)
    .to({ x: 0.47 * levelSelectPageBG.width }, 600, "Sine.easeOut")
    .to({ x: 0.46 * levelSelectPageBG.width }, 600, "Sine.easeIn", true)
    .loop();
  ...
  this.levelSelectArrowLeft.customParams = {
    btnIndex: CrackTheCircuit.BTN_INDEX_LEVEL_SELECT_PREV,
    spriteString: "levelSelectArrow",
  };
  this.game.add
    .tween(this.levelSelectArrowLeft)
    .to({ x: -0.47 * levelSelectPageBG.width }, 600, "Sine.easeOut")
    .to({ x: -0.46 * levelSelectPageBG.width }, 600, "Sine.easeIn", true)
    .loop();

  if (this.levelBlock == 1) {
    this.levelSelectArrowLeft.visible = false;
  } else if (this.levelBlock == this.totalLevelBlocks) {
...
Menu.prototype.levelBtnOnOver = function (sprite, pointer) {
  if (this.mouseDownOnLevelBoxIndex == sprite.levelNum) {
    //the mouse was down on btn and moved away and back
    sprite.loadTexture("spriteSheetMenu", "levelSelectBox_down");
    sprite.scale.setTo(0.95);
  } else if (this.isTouchScreen || (this.mouseDownOnButtonIndex != CrackTheCircuit.BTN_INDEX_LEVEL_SELECT_BOX && this.mouseDownOnButtonIndex != -1) || this.mouseDownOnLevelBoxIndex != -1) {
    //the mouse was down on another button, then moved
    //do nothing
  } else {
...
Menu.prototype.levelBtnOnUp = function (sprite, pointer, isOver) {
  ...
  this.mouseDownOnLevelBoxIndex = -1;
  sprite.scale.setTo(1);
  if (sprite.visible && isOver) {
    this.levelBoxPressed(sprite.levelNum);
  }
...
Menu.prototype.nextLevelBlockPressed = function () {
  this.levelBlock++;
  if (this.levelBlock == this.totalLevelBlocks) {
    this.levelSelectArrowRight.visible = false;
  }
...
Menu.prototype.showAboutPage = function () {
  ...
  link1.customParams = {
    btnIndex: CrackTheCircuit.BTN_INDEX_LINK_1,
    spriteString: "link1",
  };
  ...
  link2.customParams = {
    btnIndex: CrackTheCircuit.BTN_INDEX_LINK_2,
    spriteString: "link2",
  };
...
Menu.prototype.createSettingsPage = function () {
  ...
  this.settingsPageGroup.position = {
    x: this.game.world.centerX,
    y: 1.3 * this.game.world.height,
  };
  ...
  clearDataBtn.customParams = {
    btnIndex: CrackTheCircuit.BTN_INDEX_CLEAR_DATA,
    spriteString: "resetDataBtn",
  };
...
Menu.prototype.showAreYouSure = function () {
  ...
  clearDataYes.customParams = {
    btnIndex: CrackTheCircuit.BTN_INDEX_CLEAR_DATA_Y,
    spriteString: "resetDataY_btn",
  };
  ...
  clearDataNo.customParams = {
    btnIndex: CrackTheCircuit.BTN_INDEX_CLEAR_DATA_N,
    spriteString: "resetDataN_btn",
  };
...
Menu.prototype.createWinButtons = function () {
  ...
  this.replayBtn.customParams = {
    btnIndex: CrackTheCircuit.BTN_INDEX_REPLAY,
    spriteString: "button_replay",
  };
  ...
  this.nextLevelBtn.customParams = {
    btnIndex: CrackTheCircuit.BTN_INDEX_NEXT_LEVEL,
    spriteString: "button_next",
  };
...
Menu.prototype.setupRollingAnimation = function () {
  ...
  this.paperMask.beginFill(0xffffff);
  this.paperMask.drawRect(-0.5 * this.game.world.width, -0.5 * this.game.world.height, this.game.world.width, this.game.world.height);
...
Menu.prototype.startUnrollForGroup = function (group, paperMaskScaleX, paperRollDx) {
  ...
  this.paperRollLeft.position = {
    x: this.game.world.centerX,
    y: group.position.y,
  };
  this.paperRollRight.position = {
    x: this.game.world.centerX,
    y: group.position.y,
  };
...
Menu.prototype.createFreeDrawSizeButtons = function () {
  ...
  sizeBtn1.customParams = {
    btnIndex: CrackTheCircuit.BTN_INDEX_FD_SIZE_1,
    isText: true,
  };
  ...
  sizeBtn2.customParams = {
    btnIndex: CrackTheCircuit.BTN_INDEX_FD_SIZE_2,
    isText: true,
  };
  ...
  sizeBtn3.customParams = {
    btnIndex: CrackTheCircuit.BTN_INDEX_FD_SIZE_3,
    isText: true,
  };
  ...
  this.freeDrawSizeButtonGroup.forEach(function (btn) {
    btn.inputEnabled = false;
    if (btn.sizeIndex != this.freeDrawSize) {
      btn.alpha = CrackTheCircuit.FD_SIZE_BTN_ALPHA_OUT;
    }
  }, this);
};
...
Menu.prototype.fdBtnOnOver = function (sprite, pointer) {
  ...
  } else if (this.mouseDownOnButtonIndex != -1) {
    //the mouse was down on another button, then moved
    //do nothing
  } else {
    //button hover
...
Menu.prototype.fdBtnOnOut = function (sprite, pointer) {
  ...
  if (this.mouseDownOnButtonIndex == sprite.customParams.btnIndex) {
    ...
    if (sprite.sizeIndex != this.freeDrawSize) {
      sprite.alpha = CrackTheCircuit.FD_SIZE_BTN_ALPHA_OUT;
...
Menu.prototype.showPasswordPopup = function (callback, fontFamily) {
  fontFamily = fontFamily || "Arial, sans-serif";

  // Determine correct password for current level.
  let correctPassword = "key";
  if (this.gameState.curLevelNum >= 14 && this.gameState.curLevelNum <= 18) {
    const passwords = {
      14: "2e9089fefc7aaa5c45a2bb971e07f119c408f758b1b8f09cdf9c4210a0780376", //pass14
      15: "d0e0b70173324474b3da2fbf99650d8c0f2d18c7e18cc6300b74c28247e5bdc9", //pass15
      16: "9123dcbb0b42652b0e105956c68d3ca2ff34584f324fa41a29aedd32b883e131", //pass16
      17: "6e565f8f69ee2a6342d0e2847cb6e5cf4db433abecb71e2988448b098029f3e6", //pass17
      18: "1d2cafdb2fbaa2b86fe4565d2fb98940c32dc91a57f3dab5b22b84b2f105fecc", //pass18
    };
    correctPassword = passwords[this.gameState.curLevelNum] || "2c70e12b7a0646f92279f427c7b38e7334d8e5389cff167a1dc30e73f826b683"; //key
  }
  // var hashedPassword;
  // hashedPassword = CryptoJS.SHA256("pass14").toString();
  // console.log(hashedPassword);

  // Create a Phaser group for the overlay and a flag for cleanup.
  const popupGroup = this.game.add.group();
  let cleanedUp = false;
  ...
  // Create the overlay.
  const overlay = this.game.add.graphics(0, 0, popupGroup);
  ...
  // Create HTML container for the popup.
  ...
  // Add a spacer for additional images or text at the top.
  const topSpacer = document.createElement("div");
  topSpacer.style.height = "100px";
  topSpacer.style.marginBottom = "10px";

  // Create a container for custom text/images.
  ...
  if (this.gameState.curLevelNum >= 14 && this.gameState.curLevelNum <= 18) {
    ...
    topContent.innerHTML = `
      <h2>STOP</h2>
      <--<img src="assets/images/blueprint.png" alt="Sample image" style="max-width: 100%;"/>-->
  ...
  // Append the new content container into the spacer.
  topSpacer.appendChild(topContent);

  // Then append the spacer into the popup container.
  popupContainer.appendChild(topSpacer);

  // Create feedback element.
  const feedbackHTML = document.createElement("p");
  Object.assign(feedbackHTML.style, {
    fontSize: "16px",
    margin: "0 0 15px 0",
    color: "#f00",
    height: "20px",
  ...
  popupContainer.appendChild(feedbackHTML);

  // Create the password input with a placeholder.
  const inputElement = document.createElement("input");
  inputElement.type = "password";
  inputElement.placeholder = "Password..."; // Added placeholder
  Object.assign(inputElement.style, {
    ...
    boxSizing: "border-box",
    display: "block",
    margin: "0 auto 20px auto", // Centers the input horizontally
    fontFamily: fontFamily,
  });
  ...
  buttonElement.onclick = () => {
    ...
    if (userInputHash === correctPassword) {
      ...
      feedbackHTML.textContent = "Incorrect password. Reverting...";
      inputElement.value = "";
      setTimeout(() => {
        if (!cleanedUp) {
          cleanup();
  ...
  // Cleanup function to remove the DOM elements and Phaser group.
  function cleanup() {
    if (cleanedUp) return;
    cleanedUp = true;
    popupGroup.destroy();
...
menuInstance.showPasswordPopup(function (success) {
  // handle the callback
}, "Arial, sans-serif");
